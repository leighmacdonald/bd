	...
	cases := []tc{
		{
			...
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtMsg, Player: "PopcornBucketGames", Message: "I did tell you vix.", Timestamp: ts},
		},
		{
			text:     "02/24/2023 - 23:37:19: *DEAD* that's pretty thick-headed :  ty",
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtMsg, Player: "that's pretty thick-headed", Message: "ty", Timestamp: ts, Dead: true},
		},
		{
			text:     "02/24/2023 - 23:37:19: *DEAD*(TEAM) Hassium :  thats the problem vixian",
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtMsg, Player: "Hassium", Message: "thats the problem vixian", Timestamp: ts, Dead: true, TeamOnly: true},
		},
		{
			text:     "02/24/2023 - 23:37:19: ❤ Ashley ❤ killed [TrC] Nosy with spy_cicle.",
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtKill, Player: "❤ Ashley ❤", Victim: "[TrC] Nosy", Timestamp: ts},
		},
		{
		...
		},
		{
			...
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtConnect, Player: "Hassium", Timestamp: ts},
		},
		{
			text:  "02/24/2023 - 23:37:19: #    672 \"🎄AndreaJingling🎄\" [U:1:238393055] 42:57      62    0 active",
			match: true,
			expected: detector.LogEvent{
				Type: detector.EvtStatusID, Timestamp: ts, PlayerPing: 62, UserID: 672, Player: "🎄AndreaJingling🎄",
		...
		},
		{
			...
			match: true,
			expected: detector.LogEvent{
				Type: detector.EvtStatusID, Timestamp: ts, PlayerPing: 62, UserID: 672, Player: "some nerd",
		...
		},
		{
			...
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtHostname, Timestamp: ts, MetaData: "Uncletopia | Seattle | 1 | All Maps"},
		},
		{
			text:     "02/24/2023 - 23:37:19: map     : pl_swiftwater_final1 at: 0 x, 0 y, 0 z",
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtMap, Timestamp: ts, MetaData: "pl_swiftwater_final1"},
		},
		{
			text:     "02/24/2023 - 23:37:19: tags    : nocrits,nodmgspread,payload,uncletopia",
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtTags, Timestamp: ts, MetaData: "nocrits,nodmgspread,payload,uncletopia"},
		},
		{
			text:     "02/24/2023 - 23:37:19: udp/ip  : 74.91.117.2:27015",
			match:    true,
			expected: detector.LogEvent{Type: detector.EvtAddress, Timestamp: ts, MetaData: "74.91.117.2:27015"},
		},
		{
